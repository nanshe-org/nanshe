...
def wavelet_denoising(new_image, **parameters):
    ...
    for i in xrange(len(local_maxima_labeled_props)):
        ...
        # Overwrite the label parameter as it holds no information as is, always 1, Now, is the label from wavelet mask label image (No longer true.)
        ######### TODO: Revisit where this information should be stored if at all due to the fact that Label now has meaning.
